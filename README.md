...
3. One installed, run (node attackStimulator.js) in order to start the file
    ...
    d. Rollback: perform a session rollback attack on Alice's ratchet to reset values.
...
5. To perform the attack, follow these steps:
  ...
  5. Perform these actions until satisfied with the results. This showcases fresh ratchets for every file sent.
...
1. From the project root:
    ...
    b. npm install express ejs morgan method-override mongoose express-session connect-mongo connect-flash bcrypt
       i. This should connect you to a local database. If there is an error, MongoDB Compass may be required.
       ii. This will effectively create a demo cluster so that you can stimulate the web application. In order to test the application, creating an account is required.
